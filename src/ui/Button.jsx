...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      "inline-block text-sm rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-3 md:px-6 md:py-3.5 ",
  };
...
Button.propTypes = {
  ...
  to: PropTypes.string, // Assuming "to" should be a string for the Link component
